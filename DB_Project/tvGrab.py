...
#            epImgMed  = js["_embedded"]["episodes"][i]["image"]["medium"]
            ...
            cur.execute('''INSERT OR IGNORE INTO ShowTable (status, weight, updated, name, language, scheduleDay,
            scheduleTime, url, imageMed, imageOrig, premiered, summary, link, webChannel, id, network_id)
                            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''', (showStatus, showWeight, showUpdated, showName,
                                                                       showLang, showScheduleDay, showScheduleTime, showUrl, showImageMed, showImageOrig, showPremiered, showSummary, showSelfLink, showWebChannel, showId, showNetworkId))

            cur.execute('''INSERT OR IGNORE INTO EpisodeTable (id, name, airDate, url, season, imageOrig, episode,
            summary, airTime, link, airStamp, runTime, show_id)
                            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)''', (epId, epName, epAirDate, epUrl, epSeason, epImgOrig,
                                                                   epNumber, epSummary, epAirTime, epLink, epAirStamp, epRunTime, showId))

            cur.execute('''INSERT OR IGNORE INTO Genre (id, genre)
                            VALUES(?,?)''', (showId, showGenre1))

            cur.execute('''INSERT OR IGNORE INTO Network (id, networkName, runTime, timeZone, code, countryName)
                            VALUES(?,?,?,?,?,?)''', (showNetworkId, showNetworkName, showRunTime, showTimeZone, showCode, showCountryName))
